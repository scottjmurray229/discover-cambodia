...
| 9 | Author Page (`/about/scott`) | 45–60 min | #3 | ProfilePage + Person schema. Bio, credentials, content authored list. Required by Feb 2026 Core Update. |
...
| `HeritageLayout.astro` | 30–45 min | Cambodia history page template. Documentary narrative structure, historical photography. |
| `FinerThingsLayout.astro` | 30–45 min | Beer, nightlife & drinking culture pillar page. |
...
Every Cambodia map uses the same zoom/pan behavior. Implemented inline per page (not a shared component) due to page-specific CSS class prefixes.
...
Maps use Leaflet with CartoDB tiles centered on Cambodia (lat 12.5, lng 104.9). Destinations classified into 4 regions (Northwest, Northeast, South Coast, Central).
...
| History & Heritage | TouristDestination + Place + FAQPage + SpeakableSpecification | Heritage niche authority |
...
### destinations
- region (enum: northwest/northeast/south-coast/central), budgetPerDay (object), highlights (array), contentStatus, gradientColors
- v3 additions: difficulty (string), waterTemperature (string), nearbyHistorySites (array), speakableSelectors (array)
- Status workflow: `draft → review → published → needs-update`

### blog
- Categories: destination, food, festival, practical, budget

### history (Cambodia History & Heritage)
...
| `/history/[slug]` | `history/[...slug].astro` | Cambodia history & heritage pages |
...
| `/finer-things/` | `finer-things/index.astro` | Beer, nightlife & drinking culture |
