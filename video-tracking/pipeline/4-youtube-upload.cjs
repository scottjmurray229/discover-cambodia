 ...
 *   2. Create project "Discover Cambodia"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Cambodia Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'cambodia'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Cambodia ${year}`,
      description: `${title}\n\nFull guide: https://discovercambodia.info/${slug}/`,
      tags: [`Cambodia ${slug}`, 'Cambodia travel', `Cambodia ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Cambodia ✈️ #shorts`,
      description: `${title} — Full guide: https://discovercambodia.info/destinations/${slug}/`,
      tags: [`${title} Cambodia`, 'Cambodia travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Cambodia — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Cambodia for years with deep local knowledge. No sponsored content.

📖 FULL WRITTEN GUIDE: https://discovercambodia.info/destinations/${slug}/
📱 TRIP COMPANION (works offline): https://discovercambodia.info/companion/

#Cambodia #${title.replace(/\s+/g, '')} #CambodiaTravel #TravelGuide${year}`;
}
...
function generateDestTags(slug, title, dest) {
  ...
  if (dest?.region) tags.push(`${dest.region} Cambodia`);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   YOUTUBE UPLOAD — DISCOVER CAMBODIA               ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
