...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'beach': ['sihanoukville', 'koh-rong', 'kep'],
    'island': ['koh-rong', 'sihanoukville'],
    'temple': ['siem-reap', 'battambang'],
    'angkor': ['siem-reap'],
    'culture': ['siem-reap', 'phnom-penh', 'battambang'],
    'history': ['phnom-penh', 'siem-reap'],
    'food': ['phnom-penh', 'kampot', 'battambang'],
    'relax': ['kampot', 'kep', 'koh-rong'],
    'adventure': ['mondulkiri', 'kratie', 'koh-rong'],
    'family': ['siem-reap', 'phnom-penh', 'kampot'],
    'honeymoon': ['koh-rong', 'kampot', 'siem-reap'],
    'romantic': ['kampot', 'koh-rong', 'siem-reap'],
    'budget': ['phnom-penh', 'battambang', 'kampot'],
    'luxury': ['siem-reap', 'koh-rong', 'phnom-penh'],
    'nature': ['mondulkiri', 'kratie', 'tonle-sap'],
    'wildlife': ['mondulkiri', 'kratie'],
  };
  ...
  return ['phnom-penh', 'siem-reap', 'kampot'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discover Cambodia AI Trip Planner. You generate detailed day-by-day itineraries for Cambodia travel.

RULES:
- All prices in USD ($). Cambodia uses US Dollars as primary currency.
...
RESPONSE FORMAT — Return ONLY valid JSON matching this schema:
{
  ...
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Siem Reap'",
      "destination": "string — slug like 'siem-reap'",
      "items": [
        {
          ...
          "locationName": "string — specific place name for map pin, e.g. 'Angkor Wat', 'Tonle Sap Lake', 'Russian Market'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Cambodia trip:\n${parts.join('\n')}`;
