...
const SAMPLE_PASTE = `Day 1 - Arrive Phnom Penh, explore Royal Palace & Silver Pagoda
Day 2 - Tuol Sleng & Killing Fields, Russian Market, sunset at Sisowath Quay
Day 3 - Bus to Siem Reap, explore Pub Street & night market
Day 4 - Angkor Wat sunrise, Bayon, Ta Prohm
Day 5 - Banteay Srei & Tonle Sap floating village
Day 6 - Bus to Kampot, pepper farm visit, Bokor Hill Station
Day 7 - Kep crab market, fly home from Phnom Penh`;
...
export default function IntakeFlow() {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 14, opacity: 0.85, lineHeight: 1.5, maxWidth: 300 }}>
          Turn any Cambodia itinerary into an offline trip companion with local directions, budget tracking, and insider tips.
        </div>
      </div>
      ...
      <PathCard
        ...
        desc="Popular routes you can customize — Angkor, South Coast, Kampot & more"
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: COLORS.deepNight, lineHeight: 1.6, fontStyle: 'italic' }}>
          "We had zero signal on the boat to Koh Rong and the app had everything — directions, restaurant picks, even the right phrases. Felt like traveling with a local friend."
        </div>
        <div style={{ fontSize: 12, fontWeight: 700, color: COLORS.oceanTeal, marginTop: 8 }}>
          — Sarah & Mike, 7-day Cambodia trip
        </div>
      </div>
    </>
  );
  ...
  const TemplatesScreen = () => {
    ...
    const filtered = filter === 'all'
      ? templates
      : templates.filter((t) => {
          if (filter === 'northwest') return t.route.match(/Siem Reap|Battambang|Angkor/i);
          if (filter === 'south-coast') return t.route.match(/Sihanoukville|Kampot|Kep|Koh Rong/i);
          if (filter === 'northeast') return t.route.match(/Kratie|Mondulkiri/i);
          return true;
        });
    ...
    return (
      <>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16, overflowX: 'auto' as const }}>
          {['all', 'northwest', 'south-coast', 'northeast'].map((f) => (
    ...
    <>
      ...
      {!apiError && (
        <>
          ...
          {[
            { icon: '🧭', title: 'Real Directions', desc: "Not Google Maps pins — actual 'take the bus from Phnom Penh to Kampot, get off at the roundabout' directions" },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
            { icon: '💬', title: 'Local Phrases', desc: 'Khmer phrases tuned to your destinations with pronunciation' },
            { icon: '📞', title: 'Key Contacts', desc: 'Emergency numbers, hospitals, trusted local drivers & guides for your specific route' },
            { icon: '🎒', title: 'Smart Packing', desc: 'Checklist auto-generated from your activities — temple visits add modest clothing' },
            { icon: '💰', title: 'Budget Tracker', desc: 'Log expenses, see typical costs, convert currency — all offline' },
            { icon: '💵', title: 'Tipping Guide', desc: 'What to tip in every Cambodia situation, from tuk-tuk drivers to tour guides' },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
            { icon: '📶', title: 'Works Offline', desc: 'Everything cached to your phone. No signal on the boat? No problem' },
          ].map((f, i) => (
            <div key={i} style={{
